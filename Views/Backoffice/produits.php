...
$produits = $produitController->getAllAdminData($_SESSION['admin_id']);
...
<head>
    ...
    <style>
        .filter-option { cursor: pointer; transition: 0.3s; }
        .filter-option:hover { background: rgba(255,255,255,0.08) !important; color: white; }
        .filter-option.active { background: rgba(59,130,246,0.2) !important; color: var(--tech-blue); }
        .filter-option.active .filter-dot { background: var(--tech-blue); box-shadow: 0 0 6px var(--tech-blue); }
        .product-card {
...
<body class="page-anim">
            ...
            <div class="admin-content">
                <div class="admin-header-row" style="margin-bottom: 2rem; flex-wrap: wrap;">
                    ...
                    <div style="display: flex; gap: 1rem; align-items:center; flex-wrap: wrap;">
                        <span class="result-count" style="color: var(--text-muted); margin-right:1rem;"><strong><?= $totalProduitCount ?> produits</strong> total</span>
                        
                        <!-- Wrapper indispensable pour que js.js lance la fonction avec '.filter-section' -->
                        <div class="filter-section" style="margin-bottom:0; display:flex; align-items:center; flex-wrap: wrap; gap:0.5rem;">
                            <div class="filter-title" style="display:none;">Catégorie</div>
                            <div class="filter-option active" data-filter="all" style="display:inline-flex; align-items:center; border-radius:2rem; background:rgba(255,255,255,0.03); padding:0.4rem 1rem; font-size:0.85rem; border:1px solid rgba(255,255,255,0.1);">
                                <span>Tous les produits</span>
                                <span class="filter-dot" style="margin-left:8px;"></span>
                            </div>
                            <?php foreach ($categories as $category): ?>
                                <div class="filter-option" data-filter="<?= htmlspecialchars($category['nom']) ?>" style="display:inline-flex; align-items:center; border-radius:2rem; background:rgba(255,255,255,0.03); padding:0.4rem 1rem; font-size:0.85rem; border:1px solid rgba(255,255,255,0.1);">
                        ...
                        <select class="sort-select admin-btn-outline" style="background: rgba(255,255,255,0.03); color:white; border: 1px solid var(--border); font-size:0.85rem;">
                ...
                <div class="products-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 1.5rem;">
                    <?php if (empty($produits)): ?>
                        <div class="product-card" style="opacity: 0.9; width: 100%; text-align: center; padding: 4rem 2rem; grid-column: 1 / -1;">
                            <div class="card-body" style="padding:0;">
                                <div class="card-title" style="font-size:1.2rem; color:white;">Aucun produit dans la base de données</div>
                                <p style="color: var(--text-muted); margin-top: 1rem; margin-bottom:2rem;">Commencez par ajouter des produits à votre marketplace.</p>
                                <a href="ajouter_produit.php" class="admin-btn"><i class="fa-solid fa-plus"></i> Ajouter mon premier produit</a>
                            </div>
                        </div>
                    <?php else: ?>
                        ...
                        ?>
                            <div class="product-card" data-id="<?= $produit['idProduit'] ?>" style="<?= $opacityStyle ?>">
                                <div class="card-image" style="cursor: pointer; height:180px;" onclick="window.location.href='detail_produit.php?id=<?= $produit['idProduit'] ?>'">
                                    <?php if (!empty($produit['image'])): ?>
                                        <img src="<?= htmlspecialchars($produit['image']) ?>" alt="<?= htmlspecialchars($produit['nom']) ?>" style="width:100%; height:100%; object-fit: cover; border-radius: 0;" />
                                    <?php else: ?>
                                        <i class="fa-solid fa-box" style="font-size: 3rem; color:var(--text-muted);"></i>
                                    <?php endif; ?>
                                    
                                    <?php if ($produit['statut'] === 'pending'): ?>
                                        <span class="card-badge" style="background:#f59e0b; color:white; top:10px; right:auto; left:10px;">EN ATTENTE</span>
                                    <?php elseif ($produit['stock'] <= 2 && $produit['stock'] > 0): ?>
                                        <span class="card-badge badge-popular" style="top:10px; right:auto; left:10px;">STOCK FAIBLE</span>
                                    <?php elseif ($produit['stock'] <= 0): ?>
                                        <span class="card-badge badge-out" style="background:#475569; color:white; top:10px; right:auto; left:10px;">RUPTURE</span>
                                    <?php endif; ?>
                                </div>
                                <div class="card-body" style="padding:1rem;">
                                    <div class="card-category" style="margin-bottom:0.25rem;"><?= htmlspecialchars($categoryName) ?></div>
                                    <div class="card-title" style="margin-bottom:0.25rem; font-size:1.05rem;"><?= htmlspecialchars($produit['nom']) ?></div>
                                    <div class="card-rating" style="margin-bottom: 0.8rem; display:none;">
                                        <!-- Keep for JS compat -->
                                    </div>
                                    
                                    <p style="color: var(--text-muted); font-size: 0.8rem; line-height: 1.5; margin-bottom: 1rem; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; min-height:2.4rem;">
                                        <?= $descriptionPreview ?>
                                    </p>
                                    ...
                                    <div style="display: flex; gap: 0.5rem; margin-top: 1rem;">
                                        <a href="detail_produit.php?id=<?= $produit['idProduit'] ?>" class="btn-action" title="Voir l'annonce" style="display:flex; align-items: center; justify-content: center; width:36px; height:36px; background: rgba(59,130,246,0.15); color: var(--tech-blue); border-radius:0.5rem; font-weight:600; text-decoration:none;"><i class="fa-solid fa-eye"></i></a>
                                        <button type="button" class="btn-action" title="Supprimer" style="display:flex; align-items: center; justify-content: center; width:36px; height:36px; background: rgba(239,68,68,0.1); color: #ef4444; border:none; border-radius:0.5rem; font-weight:600; cursor:pointer;" onclick="openDeleteModal(<?= $produit['idProduit'] ?>)"><i class="fa-solid fa-trash"></i></button>
                                    </div>
                                </div>
                            </div>
                        <?php endforeach; ?>
    ...
    <div id="deleteModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.6); z-index:9999; align-items:center; justify-content:center; backdrop-filter:blur(5px);">
        <div style="background:var(--bg-dark); padding:2rem; border-radius:1.5rem; border:1px solid var(--border); box-shadow:0 10px 40px rgba(0,0,0,0.5); width:90%; max-width:380px; text-align:center; animation: fadeUp 0.3s ease forwards;">
            ...
            <h3 style="font-size:1.3rem; font-weight:700; margin-bottom:0.75rem; color:white;">Supprimer le produit ?</h3>
            <p style="color:var(--text-muted); font-size:0.95rem; margin-bottom:2rem; line-height:1.5;">Êtes-vous sûr de vouloir supprimer définitivement ce produit de la base de données ? Cette action est irréversible.</p>
