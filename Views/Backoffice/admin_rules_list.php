...
<main class="admin-main">
    ...
    <div class="admin-topbar animate-in">
        ...
        <div class="topbar-actions">
            <a href="admin_rules_form.php" class="btn btn-primary">
    ...
    <?php if (isset($_GET['success']) || !empty($successMessage)): ?>
        <div class="alert alert-success animate-in">
            ...
            if (isset($_GET['success'])) {
                $msgs = ['delete'=>'Règle supprimée.','toggle'=>'Statut modifié.','create'=>'Règle créée.','update'=>'Règle mise à jour.'];
    ...
    <!-- Stats rapides -->
    <div class="stats-grid animate-in delay-1" style="grid-template-columns:repeat(auto-fit,minmax(160px,1fr));margin-bottom:1.5rem;">
        <?php
        ...
        <div class="stat-card" style="--accent-color:#A855F7;">
            <div class="stat-info">
                ...
                <div class="stat-value"><?php echo $totalRules; ?></div>
            </div>
            <div class="stat-icon" style="background:rgba(168,85,247,0.15);color:#A855F7;border-radius:50%;width:48px;height:48px;">
        ...
        <div class="stat-card" style="--accent-color:#F87171;">
            <div class="stat-info">
                ...
                <div class="stat-value"><?php echo $inactRules; ?></div>
            </div>
                ...
                <?php else: ?>
                    <?php foreach ($rules as $rule): ?>
                        <tr>
                            ...
                            <td>
                                <span style="background:rgba(37,99,235,0.1);color:var(--tech-blue);padding:0.2rem 0.6rem;border-radius:var(--radius-full);font-size:0.78rem;font-weight:500;">
                            ...
                            <td>
                                <span class="badge badge-<?php echo $rule['statut'] === 'actif' ? 'actif-rule' : 'inactif'; ?>">
                                    <?php echo ucfirst($rule['statut']); ?>
                                </span>
                            </td>
                            <td style="color:var(--text-muted);font-size:0.85rem;">
                                <?php echo date('d/m/Y', strtotime($rule['date_creation'])); ?>
                            </td>
                            <td>
                                <div style="display:flex;gap:0.4rem;justify-content:flex-end;">
                                    <a href="admin_rules_form.php?action=edit&id=<?php echo intval($rule['id_rule']); ?>"
                                       class="btn btn-secondary btn-icon" title="Modifier">
                                        <i class="fa-solid fa-pen"></i>
                                    </a>
                                    <a href="admin_rules_list.php?action=toggle&id=<?php echo intval($rule['id_rule']); ?>"
                                       class="btn btn-warning btn-icon" title="Changer le statut">
                                    ...
                                    <form method="get" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'); ?>" style="margin:0;">
                                        ...
                                        <button type="submit" class="btn btn-danger btn-icon"
                                                onclick="return confirm('Supprimer cette règle ?');" title="Supprimer">
