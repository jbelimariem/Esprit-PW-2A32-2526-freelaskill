...
// Flush pour envoyer la réponse avant de supprimer
...
// ── 3. Supprimer les éphémères APRÈS envoi ───────────────────
// Cherche par contenu JSON (car statut ENUM ne contient pas 'ephemeral')
$stmt = $pdo->prepare("
...
foreach ($toCheck as $m) {
    ...
    if ($exp === 'read' && !$isSender) {
        // Destinataire ouvre → supprimer
        $toDelete[] = (int)$m['id_message'];
    } elseif ($exp === '10s' && (time() - $createdAt) >= 10) {
        $toDelete[] = (int)$m['id_message'];
    } elseif ($exp === '1h' && (time() - $createdAt) >= 3600) {
        $toDelete[] = (int)$m['id_message'];
    } elseif ($exp === '24h' && (time() - $createdAt) >= 86400) {
        $toDelete[] = (int)$m['id_message'];
    }
}
